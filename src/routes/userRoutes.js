...
router.post("/register", async (req, res) => {
  try {
    ...
    if (!tempUser) {
      return res
        .status(400)
        .json({ error: "Email not found. Please request verification first." });
    }
    ...
    if (!name || !phone || !companyName || !companyType) {
      return res.status(400).json({
        error:
          "Incomplete registration. Please provide all required information.",
      });
    }
...
router.post(
  ...
  async (req, res) => {
    try {
      // console.log("Request body:", req.body);
      const { companyName, financialYear, documentType, notes, userId, email } =
        req.body;
...
router.post("/save-reward-card2", upload.none(), async (req, res) => {
  try {
    // console.log("Request body:", req.body);
    const { platform, storeUrl, storeId, consented, rewardId, userId, email } =
      req.body;
    ...
    if (!platform || !storeUrl || consented !== "true") {
      return res.status(400).json({
        error:
          "Missing required fields. Platform, store URL, and consent are required.",
      });
    }
...
router.delete("/all-users", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: "Something went wrong while deleting users.",
  ...
  async (req, res) => {
    try {
      ...
      if (!certificateType || !expiryDate || !issueAuthority) {
        return res.status(400).json({
          error:
            "Missing required fields. Certificate type, expiry date, and issuing authority are required.",
        });
      }
...
router.get("/check-task-completion", async (req, res) => {
  try {
    ...
    if (!email || !actualTaskId) {
      return res
        .status(400)
        .json({ error: "Email and task identifier are required" });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (req.file) {
        ...
        const params = {
          Bucket: process.env.AWS_BUCKET_NAME,
          Key: `msme-certificates-reward-card/${Date.now()}-${
      ...
      if (user) {
        ...
        const rewardAmount = process.env.CARD4_REWARD
          ? parseFloat(process.env.CARD4_REWARD)
          : 100;
...
router.post("/ifscCode-verify", async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `${process.env.IFSC_VERIFY_URL}/${ifscCode}`,
      {
        timeout: 10000, // 10 second timeout
        headers: {
          Accept: "application/json",

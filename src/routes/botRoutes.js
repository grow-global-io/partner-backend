...
const STORY_STEPS = {
    1: {
        message: '🎤 Buyers love to hear your story!\n\n' +
    ...
    2: {
        message: '📦 Want to send a sample to our regional GLL Hub?\n' +
                'We\'ll help ship it!',
...
const REGISTRATION_STEPS = {
    ...
    2: {
        message: '📱 Please enter your phone number with ISD code:\nExample: +91 9400123456',
        field: 'phone'
    }
};
            ...
            if (prevStep.type === 'document') {
                ...
                if (prevStep.field === 'whatsappNumber') {
                    ...
                    if (cleanNumber.startsWith('+91')) {
                        ...
                        formattedNumber = `${cleanNumber.slice(0, codeEnd)} ${cleanNumber.slice(codeEnd)}`;
                    }
...
bot.on('message', async (msg) => {
    ...
    if (store && store.currentStep) {
        ...
        if (step.type === 'photo' && msg.photo) {
            ...
            await sendMessageWithTracking(chatId, '❌ Please upload an image for your store logo/banner.');
            return;
        }
    }
    ...
    if (registration && registration.currentStep) {
        ...
        if (msg.text) {
            await handleManufacturerStep(chatId, msg.text, registration.currentStep + 1);
            return;
        }
        // If no valid input provided for step 2
        if (registration.currentStep === 2 && !msg.photo) {
            await sendMessageWithTracking(chatId, '❌ Please upload a photo of your product.');
            return;
        }
    }
    ...
    if (verification && verification.currentStep) {
        ...
        if (step.type === 'document' && msg.document) {
            await handleVerificationStep(chatId, msg.document.file_id, currentStep + 1);
            return;
        } else if (step.type === 'text' && msg.text) {
            await handleVerificationStep(chatId, msg.text, currentStep + 1);
            return;
        } else if (step.type === 'document') {
            await sendMessageWithTracking(chatId, '❌ Please upload a document file.');
            return;
        }
    }
    ...
    if (story && story.currentStep) {
        ...
        if (step.type === 'media' && (msg.voice || msg.video)) {
            ...
            await sendMessageWithTracking(chatId, '❌ Please upload a voice note or video.');
            return;
        }
    }
    ...
    if (registrationProcess.has(chatId) && registrationProcess.get(chatId).currentStep === 0) {
    ...
    if (msg.text) {
        ...
        if (text.startsWith('/')) {
            if (!['/trader', '/manufacturer', '/createstore'].includes(text.toLowerCase())) {
                await handleCommand(msg, text);
            }
            return;
        }
        // Handle button clicks
        if (['Balance', 'Surprise', 'Clear', 'Help', 'Link', 'Register Seller', 'Create Store', 'Trader', 'Manufacturer', 'Back to Main Menu'].includes(text)) {
            await handleCommand(msg, text);
            return;
        }
    }
        ...
        try {
            ...
            const sellersWithProducts = sellers.filter(seller => {
                return seller.products.some(product => 
                    product.productType === cleanCategory
                );
            });
                ...
                for (const product of categoryProducts) {
                    ...
                    await sendMessageWithTracking(chatId,
                        'What would you like to do?',
                        {
                            reply_markup: {
                                inline_keyboard: [[
                                    { text: '📦 Request Sample', callback_data: `sample_${seller.email}` },
                                    { text: '💬 Chat', callback_data: `chat_${seller.email}` },
                                    { text: '📃 Start Deal', callback_data: `deal_${seller.email}` }
                                ]]
                            }
                        }
                    );
...
async function generateShippingLabel(seller, user) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            doc.moveDown(0.5);
            doc.fontSize(20)
               .text(shippingCode, { align: 'center' });
            ...
            // Add footer
            doc.fontSize(10)
               .text(`Here is your Shipping label ${shippingCode}`, {
                   align: 'center',
                   bottom: 50
               });
...
bot.on('message', async (msg) => {
    ...
    if (weeklyProduct && weeklyProduct.currentStep) {
        ...
        // Handle text input for other steps
        if (msg.text) {
            await handleWeeklyProductStep(chatId, msg.text, weeklyProduct.currentStep + 1);
            return;
        }
        // If no valid input provided for step 2
        if (weeklyProduct.currentStep === 2 && !msg.photo) {
